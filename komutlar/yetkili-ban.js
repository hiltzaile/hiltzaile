...
if(!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send(`**👎 Bu Komutu Kullanmak İçin __Üyeleri Yasakla__ Yetkisine Sahip Olmalısın.**`)
...
.setColor("BLUE")
.setDescription(` \`👽\` Bir Üye **Banlandı(adam öldü amınakoyim)!**;\n 
**\`🔨\` Banlayan**: ${message.member.displayName} - \`${message.author.id}\`\n
**\`🔨\` Banlanan**: ${MEMBER.user.tag} - \`${MEMBER.user.id}\`\n
**\`🔨\` Sebep**: \`${REASON}\``)
.setImage('https://cdn.discordapp.com/attachments/705894998112993332/707576797562404864/BAN.jpg')
